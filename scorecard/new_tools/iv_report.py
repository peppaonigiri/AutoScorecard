...
class IVCalculator:
    ...
    def calculate_iv(self, x, y, bins=None):
        ...
        details_df['badCumRate'] =  details_df['bad'].cumsum() / details_df['bad'].sum()
        details_df['goodCumRate'] = details_df['good'].cumsum() / details_df['good'].sum()
        ks = max(details_df.apply(lambda x: abs(x.badCumRate - x.goodCumRate), axis=1))
        details_df['ks'] = ks
    ...
    def iv_report(self, use_thread=False, max_workers=4):
        ...
        iv_df.reset_index(names=['var_name'], inplace=True)
        self.iv_df = iv_df
        return iv_df
