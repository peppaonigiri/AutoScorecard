...
class RulesFromTree:
    ...
    process_set(): 处理规则集表现。单独调用设置rule_df为空,是评估任意rule_list的效果模式。
    ...
    def process_rule(self, rule, train_badrate, test_badrate, all_badrate, train_total, test_total, all_total):
        ...
        # 防止除0错误，给为0的比例加上1e-10
        if badrate_test == 0:
            badrate_test = 1e-10
        if badrate_train == 0:
            badrate_train = 1e-10
        psi = (badrate_test-badrate_train) * np.log(badrate_test/badrate_train)
    ...
    def process_rule_wrapper(self, args):
        return self.process_rule(*args)

    def make_rules_df(self,use_thread=False,max_workers=-1,rule_list=None):
        """
        生成提取的规则在训练集和测试集和整体上的表现df。

        参数:
        use_thread (bool): 是否使用多线程。
        max_workers (int): 最大线程/进程数，如果为-1则使用系统的 CPU 核心数。
        rule_list (list): 如果为空则运行决策树提取规则。如果不为空则使用输入的规则列表。
        ...
        if use_thread:
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                ...
                # 使用 tqdm 监控任务完成的进度
                for future in tqdm(as_completed(futures), total=len(futures), desc='子线程处理进度'):
    ...
    def process_set(self, df, rule_list, rules_df=None, strategy=1, desc='all'):
        """
        处理单个数据集。
        ...
        rule_list (list): 规则列表。
        rules_df (pandas.DataFrame): 规则在训练集和测试集和整体上的表现df。若为None,进入评估rule_list在df上的表现模式。
        ...
        # 生成规则集并计算新增样本数
        for rule in tqdm(rule_list, desc=f'生成规则集_{desc}'):
            if rules_df is None:
                lift_reduce = 0
            else:
                try:
                    ...
                    # 若规则在rules_df中不存在,则lift_reduce为空
                    lift_reduce = None
        ...
        rules_set_df.columns = ['规则', '新增总样本数', '新增坏样本数', '新增好样本数', '单规则Lift衰减']
        ...
        cumulative_sum['坏样本比例降幅'] = 1 - cumulative_sum['策略后整体坏样本比例'] / all_badrate
    ...
    def make_rules_set(self, strategy=1, sort_by='test', lift_diff=None, psi_cut=None, rule_list=None):
        ...
        rule_list (list): 指定叠加顺序的规则列表,此时不要求已有self.rules_df,若为None则按指定策略排序,此时必须要有self.rules_df。
        ...
        if isinstance(rule_list, list):
            ...
            if isinstance(lift_diff, float):
                ...
                rules_df = rules_df[rules_df['lift_衰减'] <= lift_diff]
            ...
            if isinstance(psi_cut, float):
                ...
                rules_df = rules_df[rules_df['PSI'] <= psi_cut]
        ...
        self.rules_set_train = self.process_set(self.df.loc[self.train_df.index], temp_rules, rules_df, strategy=0, desc='train')
        self.rules_set_test = self.process_set(self.df.loc[self.test_df.index], temp_rules, rules_df, strategy=0, desc='test')
        ...
        # 计算规则集在训练集-测试集的Lift衰减
        self.rules_set_all['规则集Lift衰减'] = self.rules_set_train['累计Lift'] - self.rules_set_test['累计Lift']
    ...
    def plot_tree_by_dtreeviz(self, clf=None, train_df=None, train_y=None, x_list=None, num_of_trees=0, single_sample=None, viz_params=None, save_path=None, show_img=2):
        '''
        使用 dtreeviz 绘制决策树,需要先安装Graphviz,并配置环境变量,例如添加"D:/Program Files/Graphviz/bin"到系统变量Path中。
        ...
        single_sample (pandas.DataFrame): 单个样本在某棵树上的决策过程,若为None则不显示。
...
# 指定规则列表生成单规则表现df和对应的规则集表现df
# rule_list = ['x1 <= 0.5', 'x2 > 0.5', 'x3 <= 0.5']
# rf.make_rules_df(use_thread=True, max_workers=20,rule_list=rule_list)
# 此时生成的和rules_set_all是根据rf.make_rules_df生成的规则表现df叠加的规则集
# rf.make_rules_set(strategy=3)

# 如果未调用make_rules_df，直接调用make_rules_set，生成的rules_set_all是根据rule_list生成的规则表现df叠加的规则集
# 会缺少单规则lift衰减指标，如果已经调用过make_rules_df，如果单规则在rules_df中不存在，则lift_衰减为空
# rule_list = ['x1 <= 0.5', 'x2 > 0.5', 'x3 <= 0.5']
# rf.make_rules_set(strategy=3, rule_list=rule_list)

# 单独调用process_set生成规则集表现df
# rules_set_all = rf.process_set(rf.df, rule_list, rules_df=None, strategy=3, desc='all')
