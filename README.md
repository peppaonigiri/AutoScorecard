...
- **动态缺失值识别**：缺失率计算不再硬编码。系统根据数据集是否经历过填充操作，自动将填充值（如 -999）纳入缺失判定，保证全链路统计指标一致性。
...
1. **数据资产上传**：上传 CSV 数据集。系统自动执行数据清洗与 **L1 基础分析**（去除缺失率/方差不足/单一值过高特征），结果持久化保存，刷新不丢失。
2. **全局排除列配置**：用户可选择不参与统计与入模的字段（如 ID、Date），配置实时保存至项目，切换页面后自动恢复。
3. **缺失值动态填充**：支持自定义填充值（默认 -999），操作生成带时间戳的派生数据集分支，原始数据安全可追溯。后续统计自动将填充值纳入缺失判定。
4. **多集变量分析**：灵活定义划分比例，对比训练集/验证集与测试集 (OOT) 的 IV、PSI 稳定性指标。分析结果持久化，下次进入页面自动加载。
5. **变量筛选 L2**：基于 IV、相关性、PSI 阈值执行自动变量剔除，锁定入模最佳特征池。筛选报告与保留特征列表一并持久化。
6. **智能建模调优**：集成 **Optuna** 深度寻优算法，基于贝叶斯策略自动调参，并一键完成评分卡转换（支持 PDO/基准分调整）。
7. **专业模型报告**：后台渲染 KS/AUC 曲线图与分箱分布图，支持导出内嵌特征分析详情的 Excel 企业级报告。
...
erDiagram
    ...
    Projects {
        ...
        json exclude_cols "全局排除列配置"
        json split_config "OOT切分策略配置"
        json iv_report "持久化IV分析报告"
        json filter_result "持久化变量筛选结果"
    }
